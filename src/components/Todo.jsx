  ...
  return (
    <div className="shadow rounded-3 px-5 py-3 mt-5 mx-2 mx-lg-5">
        <div className="card-body">
                <h3 className="card-title d-flex justify-content-end">
                    { todo.title }
                    <button 
                        onClick={ () => toggleComplete(todo.id) }
                        className={`btn btn-sm ${todo.completed ? 'btn-outline-success' : 'btn-success'} ms-4 d-flex align-items-center py-1`}
                    >
                        {
                            todo.completed 
